# =====================================================================
# Project:     Bare-Metal ESP32-C3 Assembly SSD1306 TV Noise & LED ON
# Author:      agguro
# Date:        August 11, 2026
# Description: Pure assembly bare-metal driver to initialize SSD1306 OLED,
#              generate real-time pseudo-random TV static noise via LCG,
...
.L_init_loop:
    ...
    # =========================================================
    # BLOCK: MAIN APPLICATION (TV NOISE / STATIC LOOP & LED ON)
    # Enable GPIO 8 output safely, turn active-low LED ON permanently,
    # and run continuous pseudo-random TV static noise generator
    # =========================================================
.L_main_app:
    ...
    # Initialize LCG random seed in s6
    li   s6, 123456789

noise_loop:
    # Fill framebuffer with pseudo-random bytes
    la   t0, framebuffer
    li   t1, 360

.L_noise_fill:
    beqz t1, .L_noise_done
...
.L_noise_done:
    # Flush noise framebuffer to OLED
    jal  fb_flush

    # Small delay so the static is nicely viewable as TV noise
    li   t5, 30000
.L_noise_d:
    li   t6, 1
    sub  t5, t5, t6
